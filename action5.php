...
<?php
	if(isset($_POST['drop_c_id'])) {
		...
		$req = "必修";
		if ($row_check["required"] == $req) {//必修課程不可退選			
			//echo "<br>This course is compulsory and cannot be withdrawn.</br>";
			header('Location: action4.php?status=compulsory');
			exit;
		}
	    ...
	    if ($row['credits'] - $row['c_credit'] <= 9) {
	    	header('Location: action4.php?status=less than 9');
	   		exit;
		}
    } else {
		die('Error:' . mysqli_error($conn));
   	}
    //$total_credits = mysqli_fetch_assoc($result_credit)['credits'];
   ...
   $result6 = mysqli_query($conn, $sql3) or die('MySQL query error');
    ...
    //echo "<br>Successfully unselected the course!</br>";
    header('Location: action4.php?status=unselected');
    exit;
